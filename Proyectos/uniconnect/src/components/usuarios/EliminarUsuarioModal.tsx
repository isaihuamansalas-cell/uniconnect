...
type EliminarUsuarioModalProps = {
  abierto: boolean;
  usuario: UsuarioEliminable | null;
  onCerrar: () => void;
...
const confirmacionRequerida = "ELIMINAR USUARIO";
...
export default function EliminarUsuarioModal({
  ...
  async function eliminarUsuario() {
    if (!usuario || confirmacion !== confirmacionRequerida) {
      return;
    }
    ...
    try {
      ...
      if (!respuesta.ok) {
        ...
        setError(
          detalleRelaciones
            ? `${resultado.error ?? "No se pudo eliminar el usuario."} (${detalleRelaciones})`
      ...
      setMensaje(resultado.mensaje ?? "Usuario eliminado.");
      onUsuarioEliminado();
  ...
  return (
    <Modal
      ...
      descripcion="Esta accion elimina definitivamente el perfil y su cuenta de acceso."
      onCerrar={() => {
        if (!eliminando) {
          onCerrar();
        }
      ...
      <div className="space-y-5">
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-red-800 dark:border-red-900/60 dark:bg-red-950/40 dark:text-red-200">
          ...
          <p className="mt-2 text-sm">
            Si el usuario tiene vehiculos, salidas, avisos,
            emprendimientos, auditorias, notificaciones o cambios de
            configuracion, la eliminacion sera bloqueada y debera
            desactivarse.
          </p>
        </div>
        ...
        <label
          htmlFor="confirmar-eliminar-usuario"
          className="block text-sm font-medium text-slate-700 dark:text-slate-300"
        >
          Escribe {confirmacionRequerida} para confirmar
        </label>
        <input
          id="confirmar-eliminar-usuario"
          value={confirmacion}
