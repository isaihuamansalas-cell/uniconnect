...
type EditarAvisoModalProps = {
  abierto: boolean;
  aviso: AvisoEditable | null;
  onCerrar: () => void;
...
export default function EditarAvisoModal({
  abierto,
  aviso,
  onCerrar,
  onAvisoActualizado,
}: EditarAvisoModalProps) {
  ...
  useEffect(() => {
    ...
    setTipo(aviso.tipo);
    setDestinatario(aviso.destinatario);
    setAreaAcademica(aviso.area_academica ?? "");
    setCiclo(aviso.ciclo ?? "");
    setEstado(aviso.estado);
    setError("");
    setMensaje("");
  }, [aviso]);
  ...
  return (
    <Modal
      abierto={abierto}
      titulo="Editar aviso"
      descripcion="Actualiza el contenido, destinatario y estado."
      onCerrar={() => {
        if (!guardando) {
          onCerrar();
        }
    ...
    >
      <form onSubmit={actualizarAviso} className="space-y-5">
